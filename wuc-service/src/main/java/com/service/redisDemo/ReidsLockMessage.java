...
public class ReidsLockMessage
{
    private static final Logger logger = LoggerFactory.getLogger(ReidsLockMessage.class);
     ...
     */
    public boolean getLock(RedisLock redisLock)
    {
        return lock(redisLock, LOCK_TRY_TIMEOUT, LOCK_TRY_INTERVAL, LOCK_EXPIRE);
    }
     ...
     * @return true 获取成功，false获取失败
     */
    public boolean getLock(RedisLock redisLock, long timeout)
    {
        return lock(redisLock, timeout, LOCK_TRY_INTERVAL, LOCK_EXPIRE);
    }
    ...
    {
        return lock(redisLock, timeout, tryInterval, LOCK_EXPIRE);
    }
    ...
    {
        return lock(redisLock, timeout, tryInterval, lockExpireTime);
    }
     ...
     * @return true 获取成功，false获取失败
     */
    public boolean lock(RedisLock redisLock, long timeout, long tryInterval, long lockExpireTime)
    {
        try
        {
     ...
     * 释放锁
     */
    public void removeLock(RedisLock lock)
    {
